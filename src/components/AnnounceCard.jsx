    ...
    <div className='flex flex-col' >
      <div className='flex flex</div>-row'>
        <div className='flex w-2/3' onClick={toggleBodyCard}>
          <input 
            type="checkbox" 
            checked={annCard.isDisplayed} 
            readOnly
          /> 
          <span className='p-3 w-5/6 '> {annCard.AnnouncementString}</span> 
        </div>
        <div className='flex w-1/3'>
          <button className={`px-4 h-12 w-38 mx-2 border-none rounded-full ${annCard.isDisplayed ? 'bg-gray-300' : 'bg-gray-400'} text-black hover:bg-green-500 active:bg-green-700`} 
            onClick={handleUpdate}>
            {annCard.isDisplayed ? "Hide" : "Show"}
          </button>
          <div className='mx-2'/>
          <button className='px-4 h-12 w-38 mx-2  border-none rounded-full bg-gray-200 text-black hover:text-white active:text-white hover:bg-green-500 active:bg-gray-700' 
            onClick={showEdit}>
            Edit
          </button>
          <button className='px-4 h-12 w-38 mx-2  border-none rounded-full bg-red-200 text-black hover:text-white active:text-white hover:bg-red-500 active:bg-red-700' 
      ...
      <div className={`${bodyCard ? 'my-3' : 'my-0'}`}>
        {!bodyCard && editPressed && (
            <div>
                ...
                <div>
                    <button className='px-4 my-5 h-12 w-38 mx-2  border-none rounded-full bg-green-200 text-black hover:text-white active:text-white hover:bg-green-500 active:bg-green-700' 
